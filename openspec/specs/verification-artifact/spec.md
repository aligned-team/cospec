...
Defines the `verification` artifact — cospec's machine-parsed
acceptance-evidence ledger that forces observable behavior to be exercised at
the layer that matters before a change archives, because green CI does not mean
done. It specifies the checkbox grammar and `verification.md` location, the
closed `@layer` vocabulary, `(agent)`/`(human)` check owners with CI-uncatchable
flagging, evidence-required and explicit defer-with-reason discipline, the
per-type required-row facts, the verification matrix placement, and the
read-only verdict surfaced in `status --json`.

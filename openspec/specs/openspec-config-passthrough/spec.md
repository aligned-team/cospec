# openspec-config-passthrough Specification

## Purpose

`cospec config` reaches every subcommand of OpenSpec's machine-global config,
the one surface the shared passthrough helper cannot call: `openspec config` has
no `--store` (it declares a parent-level `--scope`), supports `--json` on `list`
alone, and has three subcommands that need the terminal. So cospec builds this
argv locally, never calls `resolveRoot`, splits the surface into a piped Class A
and a terminal-handover Class B, and keeps its one-JSON-document invariant by
relaying `list --json` verbatim and emitting cospec-owned `version: 1` envelopes
elsewhere. It also says out loud where cospec's own behaviour outranks the key
just written — forced `OPENSPEC_TELEMETRY=0`, and canon-generated harness files
— on stderr, so stdout stays exactly one document.

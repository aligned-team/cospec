...
- [x] 3.3 @integration (agent) the same run's human transcript -> the relayed warnings are printed, not only present in `--json`: `printWarnings` emits one `Warning: …` line per warning on both the clear-gate and `applyLegacy` transcripts; asserted in `test/contract/apply-relay-guard.test.ts` cases (c) and (d)
...
- [x] 4.8 @integration (agent) contract on the two reachable warning relays -> the main lane's clear gate rewrites 1.13.1's unread-delta-file warning to `cospec validate <name>` at exit 0, and `applyLegacy` on a schema whose `apply.requires` omits `specs` rewrites both spans of the no-delta-specs warning while the absolute `.openspec.yaml` path it embeds arrives byte-identical: contract cases (c) and (d)

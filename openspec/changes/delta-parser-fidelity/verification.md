...
- [ ] 1.2 @regression (agent) `cospec validate --strict` a change whose `## REMOVED Requirements` names a requirement absent from the living spec with a `*` bullet -> `archive/target-missing` ERROR, exit 1 (today: silently clean, exit 0)
...
- [ ] 6.2 @equivalence (agent) `mise run test:contract` against the pinned 1.11.0 binary -> unchanged and green, proving WU-1/WU-2/WU-5 are pin-independent; WU-3 and WU-4 are recorded as anticipating 1.13.1 semantics and are re-asserted against the real binary in the pin-bump change

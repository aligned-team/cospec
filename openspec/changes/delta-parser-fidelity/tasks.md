...
- [ ] 1.1 Widen `REMOVED_BULLET_RE`, `RENAMED_FROM_RE` and `RENAMED_TO_RE` in
      ...
      still records none
- [ ] 1.2 Add the bullet fixture family to
...
- [ ] 2.1 Replace the `openReq`-overwriting `FROM:`/`TO:` handling with
...
- [ ] 2.2 Stop `closeReq()` pushing a half-built RENAMED and retain each stray
...
- [ ] 2.3 Add the `deltas/unpaired-rename` ERROR rule to
...
- [ ] 2.4 Add the rename fixtures (lone trailing `FROM:`, `FROM/FROM/TO`,
...
- [ ] 4.1 Add a `hasScenarioBody` predicate (a `#### ` header counts only when
      ...
      increments neither `scenarioCount` nor `scenarioNames`
- [ ] 4.2 Gate the delta-side counter and the living-side
...
- [ ] 4.3 Append the missing-body hint to the `deltas/requirement-shape`
      scenario-count message, and verify via verification 4.1 that the ERROR
      carries it
- [ ] 4.4 Add the bodyless-scenario fixtures on both sides to the parser and
...
- [ ] 5.1 Collect `### Requirement:` blocks that sit outside all four delta
...
- [ ] 5.2 Add the `deltas/orphaned-requirement` WARNING rule with OpenSpec's
...
- [ ] 6.1 Add the repeated- and mixed-case `## MODIFIED Requirements` regression
      to the parser suite, and verify via verification 6.3 that every section's
      requirements accumulate
- [ ] 6.2 Run `mise run test` and `mise run test:contract`, and verify via
...
- [ ] 6.3 Update `docs/validation.md` and
      `apps/docs/reference/validation-rules.md` with the two new rule ids and
      the widened bullet forms, each fact on the page that owns it, and verify
      via verification 6.4 that neither page duplicates the other's fact
- [ ] 6.4 Run `mise run check`, and verify via verification 6.5 that it is green
      end to end

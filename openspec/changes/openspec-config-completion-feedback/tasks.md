...
- [x] 1.1 Determine the earliest wrapped-OpenSpec version that ships `config`,
      ...
      `OPENSPEC_VERSION_FLOOR` — finding: none of the three postdates the floor.
      `npm pack @fission-ai/openspec@1.0.0` and a grep over its `dist` shows
      `.command('config'`, `.command('completion'` and
      `.command('feedback <message>'` all registered at 1.0.0, so no row is
      added to `apps/docs/reference/commands.md`'s per-surface runtime-minimums
      table (which stays at `instructions archive` >=1.7.0 and
      `validate --archived` >=1.9.0) and the floor is unchanged
- [x] 1.2 Confirm against the real pinned binary whether `openspec schemas`
      emits `--json`, and verify by recording the answer as the decision to
      include or omit `schemas` as a `__complete` source — finding: it **does**
      (`openspec schemas --help` at 1.11.0 declares a `--json` option described
      as "Output as JSON (for agent use)", and `openspec schemas --json` emits a
      JSON array of `{name, description, artifacts}`). `schemas` is still
      omitted as a `__complete` source, for a reason independent of the `--json`
      question: no cospec command has a schema-name slot to fill.
      `cospec schema`'s first positional is a verb (`which|validate|fork|init`),
      and the type positional of `cospec new` is already served by the
      spawn-free `types` source. `design.md` is corrected to state that reason
      rather than the superseded `--json` conditional

## 2. WI-1 cospec config

- [x] 2.1 Add `apps/cli/src/commands/config.ts` with a pure argv builder that
...
- [x] 2.2 Wire the Class A subcommands through `passthroughOpenspec` with
...
- [x] 2.3 Refuse `cospec config --store <id>` and a missing subcommand with exit
      1 and named messages, and verify with unit tests asserting zero wrapped
      spawns on both paths (ledger 1.2)
- [x] 2.4 Implement the Class B terminal handover for `edit`, bare `profile`,
...
- [x] 2.5 Implement the `--json` envelopes for
...
- [x] 2.6 Implement the stderr advisory notes for `telemetry.enabled` and for
...
- [x] 3.1 Add `apps/cli/src/core/completions/spec.ts` deriving a
      ...
      table (ledger 4.1)
- [x] 3.2 Add the `bash.ts`, `zsh.ts`, and `fish.ts` renderers, and verify each
      generated script parses under `bash -n`, `zsh -n`, and `fish --no-execute`
      (ledger 4.2) — `bash -n` and `zsh -n` pass; the `fish --no-execute` leg is
      skipped wherever no `fish` binary exists (this machine and CI), which
      ledger row 4.2 records as an explicitly deferred sub-row
- [x] 3.3 Add `apps/cli/src/commands/completion.ts` with `$SHELL` basename
...
- [x] 3.4 Add `apps/cli/src/commands/complete.ts` serving `changes`, `specs`,
...
- [x] 4.1 Add `apps/cli/src/commands/feedback.ts` title, body, and provenance
...
- [x] 4.2 Implement submission via `gh issue create` with array argv,
...
- [x] 4.3 Implement the manual-submission fallback (formatted block plus
...
- [x] 4.4 Implement `--upstream` as a version-asserted piped relay with a
      ...
      (ledger 6.4)
- [x] 4.5 Implement the `--json` document with `submitted`, `url`, `title`, and
...
- [x] 5.1 Add the four `COMMANDS` entries (`config`, `completion`, `feedback`,
      ...
      "not yet implemented"
- [x] 5.2 Extend `renderCodexRules` in `apps/cli/src/harness/adapters.ts` with
...
- [x] 6.1 Land the unit suites (`config-args`, `feedback-format`, `completions`)
...
- [x] 6.2 Land `apps/cli/test/contract/config-surface.test.ts` against the real
...
- [x] 6.3 Extend `apps/cli/test/integration/pack-standalone.test.ts` to run
      `cospec config path`, `cospec completion zsh`, and
      `cospec feedback --json` (with `gh` stripped from `PATH`, so the fallback
      path runs and no issue can be filed) from the packed standalone binary,
      and verify `mise run test:pack:standalone` is green alongside
      `mise run test:pack`. `feedback --help` is deliberately not the probe:
      `cli.ts` answers `--help` from the static `COMMANDS` table before it
      consults `COMMAND_MODULES`, so it would pass even with the module dropped
      (ledger 7.1)
- [x] 6.4 Run the ledger end to end on the final branch state, recording an
...
- [x] 7.6 Settle the suspected trailing-`--no-color` hazard on the other

...
- `scripts/hooks/pre-push-gate` — detect the all-zeros remote sha while reading
  the hook's stdin ref lines and pass hk an explicit
  `--from-ref <synthetic empty root commit> --to-ref HEAD` range, so the gate
  diffs the entire tree (every file is new on a first push) instead of resolving
  a nonexistent `origin/HEAD`. (`--all` is not enough: hk 1.48 still resolves
  origin/HEAD.)

## Impact

- Local git pre-push hook behavior only; no workflows, jobs, or secrets.
- First pushes now run the gate over all files (stricter, not weaker); pushes to

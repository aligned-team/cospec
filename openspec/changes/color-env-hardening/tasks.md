...
- [ ] 1.1 Replace the `WRAPPED_ENV` overlay in `apps/cli/src/core/openspec.ts`
...
- [ ] 1.2 Add the colour-deletion comment naming the Node
...
- [ ] 2.1 Apply the same colour-variable deletion to the `spawn` helper in
...
- [ ] 2.2 Confirm the same suites still pass with those variables unset and
      verify via
      `env -u FORCE_COLOR -u COLORTERM -u CLICOLOR mise run test:contract`

## 3. Pin the behaviour and close out

- [ ] 3.1 Add a unit test asserting the builder strips all four colour variables
      and keeps the pinned keys, and verify `mise run test` is green
- [ ] 3.2 Record every verification.md row with its observed result (or an

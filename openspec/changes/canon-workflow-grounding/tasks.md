...
- [ ] 1.1 Reroute `explore.md`'s "Ground yourself first" section from the
      ...
      hits
- [ ] 1.2 Add the adapted planning-discipline block (one focused question at a
...
- [ ] 1.3 Add the capability-inventory step naming `cospec list --specs` and
...
- [ ] 1.4 Add the narrowly scoped rule that an explicit capture request is its
      own write confirmation, and verify with the canon-content assertion of row
      2.4
- [ ] 1.5 Confirm `/cospec:propose` is named at every handoff in `explore.md`
...
- [ ] 2.1 Add the adapted grounding step to `propose.md`: run
...
- [ ] 2.2 Add the root-resolution-failure branch (report, do not proceed)
...
- [ ] 3.1 Restage `update.md` step 4 from "Apply the requested edit" to drafting
...
- [ ] 3.2 Change the already-coherent branch from "edit nothing" to "propose no
...
- [ ] 4.1 Append a trigger sentence to each of the twelve workflow descriptions
...
- [ ] 4.2 Disambiguate the `cospec-update-change` description from the
      `cospec update` CLI subcommand, and verify with the assertion of
      verification row 4.2
- [ ] 4.3 Confirm the descriptions render verbatim into the claude, codex,
      opencode, and `.agents/skills` outputs, verified by the cross-harness
      comparison of verification row 4.3
- [ ] 4.4 Run the advisory e2e eval over naturally phrased prompts and record
      the selection outcomes, verified by verification row 4.4

## 5. Track E5 — artifact template top-level headings

- [ ] 5.1 Prepend `# Proposal` to both `proposal` template variants, `# Specs`
      to the specs template body, `# Design`, `# Tasks`, and `# Verification` to
      theirs, and restate `blocking-changes`' `# Dependencies` as
      `# Blocking Changes`, verified by the first-line check of verification row
      5.1 and the variant check of row 5.4
- [ ] 5.2 Add parser fixtures proving `tasks.ts` `GROUP_RE`, `verification.ts`
...
- [ ] 5.3 Capture the before/after `cospec validate <slug> --strict` equivalence
      on an untouched scaffold, verified by verification row 5.3

## 6. Track E6 — archive and bulk-archive prose

- [ ] 6.1 Add the sentence to `bulk-archive.md` recording that each
...
- [ ] 6.2 Audit `archive.md` and `sync-specs.md` against the

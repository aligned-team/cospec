...
| `proposal/sections`        | E                                       | required H2s per variant: full → `## Why`, `## What Changes`, `## Impact` (+ `## Capabilities` for feat, and fix when `specs/` exists); lite → `## Why`, `## What Changes`, `## Impact`; plus `## Surfaces` for every surfaces:true type (all but chore/docs/style/test), fail-closed even when the block is empty |

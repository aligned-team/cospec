...
- **Resolution by path, never `$PATH`.** `core/openspec.ts` resolves the wrapped
  ...
  `core/openspec-embedded.ts` extracts it write-once (atomic rename) to
  `${XDG_CACHE_HOME:-~/.cache}/cospec/openspec-<version>/vendor/bin/openspec.js`
  next to a synthesized `package.json` two levels up (the bundle reads
  `../../package.json` for its version at load time), with a byte-length
  post-condition on the extracted file. OpenSpec's built-in `schemas/` are not
  embedded: a cospec-managed project always has project-local

...
`openspec/schemas/**`, the harness files under `.claude/`,
`.agents/skills/cospec-*/`, `.codex/`, and `.opencode/`, and the gate files are
generated by `cospec init` and regenerated by `cospec update`. Every managed
file carries provenance frontmatter naming the generating version and a content
hash of its body — that's what lets `cospec update` tell an untouched file from
one you edited.

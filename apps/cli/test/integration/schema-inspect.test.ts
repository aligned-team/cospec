...
describe('cospec schema fork/init — legacy lane, project-local schemas', () => {
  test('fork creates a project schema dir + schema.yaml on disk', async () => {
    const dest = join(root, 'openspec/schemas/forked-schema')
    expect(existsSync(dest)).toBe(false)
    const res = await cospec(['schema', 'fork', 'feat', 'forked-schema'], { cwd: root })
    expect(res.exitCode).toBe(0)
    expect(existsSync(join(dest, 'schema.yaml'))).toBe(true)
  })

  test('init creates a project schema dir + schema.yaml on disk', async () => {
    const dest = join(root, 'openspec/schemas/new-schema')
    const res = await cospec(
      ['schema', 'init', 'new-schema', '--description', 'a custom workflow'],
      { cwd: root },
    )
    expect(res.exitCode).toBe(0)
    expect(existsSync(join(dest, 'schema.yaml'))).toBe(true)
  ...
  test('init refuses a reserved canon name before touching disk', async () => {
    const canonFeatSchema = join(root, 'openspec/schemas/feat/schema.yaml')
    const before = readFileSync(canonFeatSchema, 'utf8')
    const res = await cospec(['schema', 'init', 'feat'], { cwd: root })
    expect(res.exitCode).toBe(1)
    expect(res.stderr).toMatch(/canon cospec types/)
    expect(readFileSync(canonFeatSchema, 'utf8')).toBe(before)
  })

  test('fork refuses an explicit reserved-name destination before touching disk', async () => {
    const canonFeatSchema = join(root, 'openspec/schemas/feat/schema.yaml')
    const before = readFileSync(canonFeatSchema, 'utf8')
    const res = await cospec(['schema', 'fork', 'chore', 'feat'], { cwd: root })
    expect(res.exitCode).toBe(1)
    expect(res.stderr).toMatch(/canon cospec types/)
    expect(readFileSync(canonFeatSchema, 'utf8')).toBe(before)
  })

# cospec — agent configuration

This file is for non-Claude-Code runtimes (Codex, OpenCode, etc.). Claude Code
users: read [CLAUDE.md](CLAUDE.md) for the full picture including
...
never `$PATH`, accepting `>=1.3.1 <2.0.0`, pinned to 1.5.0 for dev/CI) and adds:
...
| Wrapped tool  | `@fission-ai/openspec` 1.5.0 (pin; `>=1.3.1 <2.0.0` accepted)    |
...
accepted range `>=1.3.1 <2.0.0` (dev/CI pins 1.5.0).
...
project (OpenCode) or start a new session (Codex) after `cospec init`/`update`.
